...
void CodeGen::Generate(ZTypeRegistry* p_Registry, const std::filesystem::path& p_OutputPath)
{
	...
	m_ClassesFile.open(p_OutputPath / "classes.txt", std::ofstream::out);
	m_EnumsFile.open(p_OutputPath / "enums.txt", std::ofstream::out);

	m_SDKEnumsHeader.open(p_OutputPath / "Enums.h", std::ofstream::out);
	...
	m_SDKEnumsHeader << " *" << std::endl;
	m_SDKEnumsHeader << " * If you wish to regenerate this file because of a game update use the ZHMTools CodeGen tool:" << std::endl;
	m_SDKEnumsHeader << " * https://github.com/OrfeasZ/ZHMTools/releases" << std::endl;
	m_SDKEnumsHeader << " */" << std::endl;
	m_SDKEnumsHeader << std::endl;
	...
	m_ReflectiveClassesHeaderFile << " *" << std::endl;
	m_ReflectiveClassesHeaderFile << " * If you wish to regenerate this file because of a game update use the ZHMTools CodeGen tool:" << std::endl;
	m_ReflectiveClassesHeaderFile << " * https://github.com/OrfeasZ/ZHMTools/releases" << std::endl;
	m_ReflectiveClassesHeaderFile << " */" << std::endl;
	m_ReflectiveClassesHeaderFile << std::endl;
	...
	m_ReflectiveClassesSourceFile << " *" << std::endl;
	m_ReflectiveClassesSourceFile << " * If you wish to regenerate this file because of a game update use the ZHMTools CodeGen tool:" << std::endl;
	m_ReflectiveClassesSourceFile << " * https://github.com/OrfeasZ/ZHMTools/releases" << std::endl;
	m_ReflectiveClassesSourceFile << " */" << std::endl;
	m_ReflectiveClassesSourceFile << std::endl;
	...
	for (auto& s_TypeDef : p_Registry->m_types)
	{
		++s_ProcessedTypes;
...
void CodeGen::GenerateClass(STypeID* p_Type)
{
	...
	for (uint16_t i = 0; i < s_Type->m_nInterfaceCount; ++i)
	{
		...
		s_Stream << "\tpublic " << s_Type->m_pInterfaces[i].m_pType->typeInfo()->m_pTypeName << ", // Interface offset 0x" << std::hex << s_Type->m_pInterfaces[i].m_nOffset << " " << std::dec << std::endl;
	}
	...
	for (uint16_t i = 0; i < s_Type->m_nBaseClassCount; ++i)
	{
		...
		s_Stream << "\tpublic " << s_Type->m_pBaseClasses[i].m_pType->typeInfo()->m_pTypeName << ", // Base class offset 0x" << std::hex << s_Type->m_pBaseClasses[i].m_nOffset << " " << std::dec << std::endl;
	}
	...
	for (uint16_t i = 0; i < s_Type->m_nConstructorCount; ++i)
	{
		auto s_Constructor = s_Type->m_pConstructors[i].m_pInfo;

		s_Stream << "\t" << s_Type->m_pTypeName << "(";

		for (uint64_t j = 0; j < s_Constructor->m_nArgumentCount; ++j)
		{
			auto s_Arg = s_Constructor->m_pArgType[j];

			if (!s_Arg.typeInfo()) {
				s_Stream << "void* p_Arg" << j;
			}
			else {
				s_Stream << s_Arg.typeInfo()->m_pTypeName << " p_Arg" << j;
			}

			if (j < s_Constructor->m_nArgumentCount - 1)
				s_Stream << ", ";
		}

		s_Stream << ");";

		if (s_Constructor->m_pReturnType && s_Constructor->m_pReturnType->typeInfo())
			s_Stream << " // Returns " << s_Constructor->m_pReturnType->typeInfo()->m_pTypeName;
	...
	for (uint16_t i = 0; i < s_Type->m_nPropertyCount; ++i)
	{
		...
		std::string s_PropName = GetPropName(s_Prop);
		...
		if (!s_Prop.m_pType->typeInfo())
		{
			...
			s_Stream << "\tchar " << s_PropName << "[0x" << std::hex << std::uppercase << s_PropSize << std::dec << "];";

		}
		else
		{
			s_Stream << "\t" << s_Prop.m_pType->typeInfo()->m_pTypeName << " " << s_PropName << ";";
		}
	...
	for (uint16_t i = 0; i < s_Type->m_nInputCount; ++i)
	{
		...
		s_Stream << "\t// Input pin: (0x" << std::hex << std::uppercase << s_Input.m_nPinID << std::dec << ")" << std::endl;
	}
...
void CodeGen::GenerateReflectiveClass(STypeID* p_Type)
{
	...
	s_HeaderStream << s_Indent << "class /*alignas(" << static_cast<int>(s_Type->m_nTypeAlignment) << ")*/ " << s_NormalizedName;
	...
	s_HeaderStream << s_Indent << "\tstatic void Serialize(void* p_Object, ZHMSerializer& p_Serializer, zhmptr_t p_OwnOffset);" << std::endl;
	s_HeaderStream << s_Indent << "\tstatic bool Equals(void* p_Left, void* p_Right);" << std::endl;
	s_HeaderStream << s_Indent << "\tstatic void Destroy(void* p_Object);" << std::endl;
	s_HeaderStream << std::endl;
	...
	for (uint16_t i = 0; i < s_Type->m_nPropertyCount; ++i)
	{
		...
		else
		{
			...
			if (NormalizeName(s_Prop.m_pType) == "TArray")
				return;
			
			s_HeaderStream << s_Indent << "\t" << NormalizeName(s_Prop.m_pType) << " " << s_PropName << ";";
		}
	...
	s_SourceStream << "{" << std::endl;
	s_SourceStream << "\tauto* s_Object = reinterpret_cast<" << s_NormalizedName << "*>(p_Object);" << std::endl;
	s_SourceStream << "\ts_Object->~" << s_NormalizedName << "();" << std::endl;
	s_SourceStream << "}" << std::endl;
	s_SourceStream << std::endl;
...
void CodeGen::GeneratePropertyNamesFiles()
{
	...
	m_PropertyNamesHeaderFile << " *" << std::endl;
	m_PropertyNamesHeaderFile << " * If you wish to regenerate this file because of a game update use the ZHMTools CodeGen tool:" << std::endl;
	m_PropertyNamesHeaderFile << " * https://github.com/OrfeasZ/ZHMTools/releases" << std::endl;
	m_PropertyNamesHeaderFile << " */" << std::endl;
	m_PropertyNamesHeaderFile << std::endl;
	...
	m_PropertyNamesSourceFile << " *" << std::endl;
	m_PropertyNamesSourceFile << " * If you wish to regenerate this file because of a game update use the ZHMTools CodeGen tool:" << std::endl;
	m_PropertyNamesSourceFile << " * https://github.com/OrfeasZ/ZHMTools/releases" << std::endl;
	m_PropertyNamesSourceFile << " */" << std::endl;
	m_PropertyNamesSourceFile << std::endl;
...
void CodeGen::GenerateEnumsFiles()
{
	...
	m_EnumsHeaderFile << " *" << std::endl;
	m_EnumsHeaderFile << " * If you wish to regenerate this file because of a game update use the ZHMTools CodeGen tool:" << std::endl;
	m_EnumsHeaderFile << " * https://github.com/OrfeasZ/ZHMTools/releases" << std::endl;
	m_EnumsHeaderFile << " */" << std::endl;
	m_EnumsHeaderFile << std::endl;
	...
	m_EnumsSourceFile << " *" << std::endl;
	m_EnumsSourceFile << " * If you wish to regenerate this file because of a game update use the ZHMTools CodeGen tool:" << std::endl;
	m_EnumsSourceFile << " * https://github.com/OrfeasZ/ZHMTools/releases" << std::endl;
	m_EnumsSourceFile << " */" << std::endl;
	m_EnumsSourceFile << std::endl;
