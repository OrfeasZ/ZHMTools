...
void OutputNavMesh_HUMAN_Print(NavPower::NavMesh* p_NavMesh)
{
	...
	for (auto s_Section: p_NavMesh->m_aSections)
	{
        printf("==== NavMesh Section Header ====\n");
	    printf("Sect_id: %x\n", s_Section.m_hdr->m_id);
	    printf("Sect_size: %x\n", s_Section.m_hdr->m_size);
	    printf("Sect_pointerSize: %x\n", s_Section.m_hdr->m_pointerSize);

	    printf("==== NavSet Header ====\n");
	    printf("Set_endianFlag: %x\n", s_Section.m_setHdr->m_endianFlag);
	    printf("Set_version: %x\n", s_Section.m_setHdr->m_version);
        ...
        for (auto s_NavGraph : s_Section.m_aNavGraphs) {
            ...
            printf("kdData_bbox.min.X: %f\n", s_NavGraph.m_kdTreeData->m_bbox.m_min.X);
            printf("kdData_bbox.min.Y: %f\n", s_NavGraph.m_kdTreeData->m_bbox.m_min.Y);
            printf("kdData_bbox.min.Z: %f\n", s_NavGraph.m_kdTreeData->m_bbox.m_min.Z);
            printf("kdData_bbox.max.X: %f\n", s_NavGraph.m_kdTreeData->m_bbox.m_max.X);
            printf("kdData_bbox.max.Y: %f\n", s_NavGraph.m_kdTreeData->m_bbox.m_max.Y);
            printf("kdData_bbox.max.Z: %f\n", s_NavGraph.m_kdTreeData->m_bbox.m_max.Z);
            printf("kdData_size: %x\n", s_NavGraph.m_kdTreeData->m_size);

            uintptr_t s_curPointer = (uintptr_t)s_NavGraph.m_rootKDNode;
            uintptr_t s_endPointer = s_curPointer + s_NavGraph.m_kdTreeData->m_size;
            uintptr_t s_navGraphStart = (uintptr_t)s_NavGraph.m_hdr;
...
{
	NavPower::NavMesh s_NavMesh = *p_NavMesh;
    for (auto s_Section : s_NavMesh.m_aSections)
    {
        int s_NavGraphIndex = 0;
        for (auto s_NavGraph : s_Section.m_aNavGraphs)
        {

	        printf(
		        "Areas['%s']['%d'] = [[%f, %f, %f, %f, %f, %f],", s_FileName.c_str(), s_NavGraphIndex,
		        s_NavGraph.m_hdr->m_bbox.m_min.X, s_NavGraph.m_hdr->m_bbox.m_min.Y, s_NavGraph.m_hdr->m_bbox.m_min.Z,
		        s_NavGraph.m_hdr->m_bbox.m_max.X, s_NavGraph.m_hdr->m_bbox.m_max.Y, s_NavGraph.m_hdr->m_bbox.m_max.Z);
	        ...
	        std::map<uint32_t, std::vector<std::pair<uint32_t, NavPower::BBox>>> s_kdTreeDepthToSplitAndBBoxesMap = s_NavGraph.ParseKDTree();
